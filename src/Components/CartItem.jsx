...
export default function CartItem({ item }) {
  const { deleteProduct, UpdataCounter, disabled } = useContext(CartContextVar);
  const [count, setCount] = useState(item?.count);
  const [showConfirm, setShowConfirm] = useState(false);  

  function handleKeyDown(e) {
    if (e.key === "Enter") {
      UpdataCounter(item._id, Number(count)); 
    }
  }

  function updateCountIfChanged() {
    if (count === item?.count) return;
    UpdataCounter(item.product._id, count);
  }
  ...
  return (
    <div>
      <div className="  h-full  mb-6 rounded-lg   gap-6    flex  justify-center items-center  ">
        ...
        {/* quantity */}
        <div className="flex items-center  border-4 border-gray-200  rounded-2xl  text-2xl">
          <button
            disabled={disabled}
            onClick={() => {
              const newCount = Math.max(1, item?.count - 1);
              setCount(newCount);
              UpdataCounter(item.product._id, newCount);
            }}
            className="disabled:cursor-not-allowed cursor-pointer   text-3xl py-1 px-3.5 duration-100 text-darkPrimary hover:text-primary "
          >
            -
          </button>

          <input
            className="h-8 w-12   font-bold text-darkPrimary   text-center text-2xl outline-none"
            type="number"
            value={count}
            onChange={(e) => setCount(Number(e.target.value))}
            onKeyDown={handleKeyDown}
            onBlur={updateCountIfChanged}
            min={1}
          />
